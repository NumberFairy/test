	...
	public void page(PageEntity<BaseAlumniBranchApply> pageEntity, Map<String, Param> params, List<Order> orders) {
		...
		for (BaseAlumniBranchApply apply : list) {
			...
			if (alumni.getGender() != null) {
				GenderEnum geEnum = GenderEnum.valueByIndex(alumni.getGender());
				apply.setGenderStr(geEnum != null ? geEnum.getName() : "");
			}
			BaseAlumniBranch bs = clubSrv.find(apply.getBranchId());
			apply.setBranchName(bs != null ? bs.getName() : "");
			if (alumni.getCollege() != null) {
