...
public class BaseIdentityRelationDaoImpl extends SimpleHibernateDao<BaseIdentityRelation, String> implements BaseIdentityRelationDao {
	...
	public void page(PageEntity<BaseIdentityRelation> pageEntity, Map<String, Param> params, List<Order> orders) {
		PageEntity<BaseAlumni> pageAlumni = new PageEntity<>(pageEntity.getStart(), pageEntity.getPageSize());
		Map<String, Object> map = buildParams(params);
		pageBySql(selSql(map), pageAlumni, queryExtension(), map);
		List<BaseAlumni> alumniList = pageAlumni.getResults();
		List<BaseIdentityRelation> list = Lists.newArrayList();
		for (BaseAlumni alumni : alumniList) {
			BaseIdentityRelation relation = this.findUniqueBy("alumniId", alumni.getId());
			if (null != relation) {
				list.add(relation);
			}
		}
		pageEntity.setResults(list);
		pageEntity.setCurrentPage(pageAlumni.getCurrentPage());
		pageEntity.setCurrentPageLength(pageAlumni.getCurrentPageLength());
		pageEntity.setTotalPages(pageAlumni.getTotalPages());
	}
