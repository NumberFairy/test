 ...
 * */
 ...
 $(function(){
   ...
   buildUeditor('editor_id');
        ...
        if(typestr){
         initSel(typestr.replace(/\s/g, "").split(','));
       }
 ...
 $.each(data, function(i, obj) {
  $.each(obj.children, function(j, child) {
   var selected;
   var p=child.uploadImg+'$'+child.uploadVideo+'$'+child.uploadFile;
   $.each($('#typestr').val().split(','), function(m, checkId){
    if (child.menuId == checkId) {
     h += '<option data-id="'+p+'" value="' + child.menuId + '" selected>' + child.name + '</option>';
     selectMenus.push(child);
     selected = true;
   }
 });
   if (!selected) {
    h += '<option data-id="'+p+'" value="' + child.menuId + '">' + child.name + '</option>';
  }
  childMenus.push(child);
});
});
 $('#select-type').html(h).chosen();
}
})
}
function initSel(arr){
  selectMenus.splice(0,selectMenus.length);
  $.each(arr, function(i) {
    selectMenus.push(findObj(arr[i]));
  })
  fillDivShowArray();
}
