...
public class AlumniAction extends AlumniBaseAction<BaseAlumni> {
	 ...
	 * 功能：导出
	 * 
	 * @author debug by jfzhai@wisdombud.com
	 *         2017年4月28日
	 */
	public void exportInfo() {
		List<BaseAlumni> results = Lists.newArrayList();
		if (StringUtils.isNotEmpty(this.id)) {
			results = exportSelectInfo(this.id);
		} else {
			results = alumniSrv.findBy(this.buildParams(), this.buildOrder());
		}

		alumniSrv.createExcelWorkBookInfo(response, results, exportFields);
	}

	private List<BaseAlumni> exportSelectInfo(String id) {
		String[] ids = id.split(",");
		Map<String, Param> map = new HashMap<>();
		map.put("id", Param.put(Param.IN, ids));
		return alumniSrv.findBy(map, this.buildOrder());
	}
